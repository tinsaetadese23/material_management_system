    ...
    RequestDispatcher ds = request.getRequestDispatcher("../ATCS");
...
<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
  ...
  <div class="content-wrapper">
    ...
    <c:forEach var="roles" items ="${userRoleLists}">
    	<c:if test = "${roles.create == 1 && roles.pageIdNameHolder == 'ASSTRN'}">
    	<c:set var="create" value="1" />
    	</c:if>
    	<c:if test = "${roles.update == 1 && roles.pageIdNameHolder == 'ASSTRN'}">
    	<c:set var="update" value="1" />
    	</c:if>
    	<c:if test = "${roles.delete == 1 && roles.pageIdNameHolder == 'ASSTRN'}">
    	<c:set var="delete" value="1" />
    	</c:if>
    </c:forEach>
          <%if(request.getAttribute("efound") != null){%>
                <script type="text/javascript">
                window.onload = function(){
               // document.getElementById('custom-tabs-two-settings-tab').style.display="none";
            ...
            <div class="card card-tabs">
              <div class="card-header custom_card_branding p-0 pt-1">
                <ul class="nav nav-tabs" id="custom-tabs-two-tab" role="tablist">
                  <li class="pt-2 px-3"><h3 class="card-title">Asset Transfer</h3></li>
              ...
              <div class="card-body">
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  <div class="tab-pane fade show active" id="custom-tabs-two-home" role="tabpanel" aria-labelledby="custom-tabs-two-home-tab">
    					<form method="post" action = "../ATCS/tag">
                  ...
                  <div  class="tab-pane fade" id="custom-tabs-two-settings" role="tabpanel" aria-labelledby="custom-tabs-two-settings-tab">
				  	<form action = "../ATCS/tag" method = "post">
				  	  <div class="row">
				  	  	<div class="col-md-6">
  						 ...
  						 <div class="form-group">
    						...
    						<input type ="hidden" value = "<c:out value ="${emp.id}"/>" name = "pempid"/>
				  	  	...
				  	  	<div class="col-md-6">
				  	  	 <p>Transfer To</p>
				  	  	 <input  value="<c:out value ="${ast.tagNumber}"/>" type ="hidden" name ="tag"/>
				  	 	 </hr>
				  	  		 <div id="div22" class="form-group">
    						<label for="some">Employee Id</label><br>
    						<input value="<c:out value ="${emp2.id}"/>" style="display:inline-bock" name="empId" type="text" class="" id = "assetClass" required>
    						<br><label for="some">Branch Code</label><br>
    						<input value="<c:out value ="${emp2.branch}"/>" style="display:inline-bock" name="brncode" type="text" class="" id = "assetClass" required>
  						 ...
  						 <div class="form-group">
    						<hr>
    						<p><strong>ID :</strong></p><c:out value="${emp2.id}"></c:out>
    						<p><strong>Name :</strong></p><c:out value="${emp2.fullName}"></c:out>
    						<p><strong>Branch :</strong></p>
    						<c:out value="${emp2.branch}"></c:out>
    						<p><strong>Dep't :</strong></p>
    						<c:out value="${emp2.dept}"></c:out>
    						</div>
    					...
    					<c:if test = "${create == '1'}">
    					<input onclick="checkMe();" value="Transfer"  class="btn btn-primary" type="submit" class="form-control" id = "assetClass" />
						</c:if>
    				...
    				<form>
    					...
    					<table class="table table-bordered">
	    					...
	    					<tr>
		    					...
		    					<td><a href = "../ATCS/view?id=<c:out value ="${stc.id}"/>"><i class="far fa-eye"></i></a></td>
		    					<c:if test = "${delete == '1'}">
		    					<td><a onclick="return deleteMe()" href = "../ATCS/delete?id=<c:out value ="${stc.id}"/>"><i class="fas fa-times btn-danger"></i></a></td>
								</c:if>
	    					</tr>
	    					</c:forEach>
	    					<%}%>
        ...
        <div class="modal-dialog modal-lg">
          <div class="modal-content">
            ...
            <div class="modal-body">
   			 <form action = "../ATCS/auth" method="POST">
   			 <input name="id" type="hidden" value="<c:out value="${objprp.id}"/>"/>
   			 <div class="row">
   			 	<div class="col-md-4">
   			 	<p><strong>Sub Category</strong> : <c:out value="${objprp.comment}"></c:out></p>
   			 	<p><strong>Item Desc</strong> : <c:out value="${objprp.item}"></c:out></p>
   			 ...
   			 <c:if test = "${objprp.authStatus == 'UTRN' }">
            ...
            <div class="modal-footer justify-content-between">
              ...
              <c:if test = "${objprp.authStatus == 'ATRN' }">
